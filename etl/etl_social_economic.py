...
def fetch_all_records() -> Tuple[List[Dict[str, Any]], Dict[str, str]]:
    ...
    with requests.Session() as session:
        session.headers.update({
            "User-Agent": "etl-social-economic/1.0",
...
def transform_row(
    ...
    cluster_2021 = clean_integer(row.get(column_id_map["אשכול 2021"]))
...
def transform_rows(
    ...
    deduped: Dict[int, Tuple[int, Optional[int], int]] = {}
    errors: List[str] = []
    duplicate_count = 0
...
CREATE_TABLE_SQL = """
CREATE TABLE IF NOT EXISTS social_economic (
...
DO UPDATE SET
    cluster_2021 = EXCLUDED.cluster_2021,
...
def load_rows(conn, rows: List[Tuple[int, Optional[int], int]]) -> None:
    ...
    conn.commit()
    print(f"[INFO] loaded {len(rows)} rows into social_economic")
...
def main() -> None:
    try:
        print("[INFO] starting ETL social_economic")
